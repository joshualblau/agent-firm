...
- **High-risk deep pass:** for high-blast-radius changes, the Lead may run a second reviewer on **Fable 5** (strongest bug-finder) for the correctness / maintainability lenses. Do **not** put the **security_privacy** lens on Fable — its safety classifiers can refuse security-adjacent work; keep the security lens on Opus.
