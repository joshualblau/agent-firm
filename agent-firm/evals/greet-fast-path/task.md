...
node:test. Open a run ledger with `bin/new-run`, delegate to the `implementer` and `qa-tester`
subagents, produce a schema-valid `08-qa-verdict.json` validated by `bin/validate-verdict`, and
