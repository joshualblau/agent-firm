...
UNDOCUMENTED_CONTROLS = {
    # RE-MEASURED 2026-09-03 on claude 2.1.258. This is the SECOND time staleness has forced a
    # re-measurement here (2026-08-25 on 2.1.234 was the first), and both times the rule worked
    # exactly as intended: the installed CLI moved, the recorded version no longer matched, and the
    # scanner refused the record as evidence rather than letting an old observation stand in for a
    # current one. That the answer has not changed across three builds is not a reason to relax it
    # -- it is the only reason we KNOW the answer has not changed.
    #
    ...
    #
    # Measured 2026-09-03, claude 2.1.258, both commands run back to back:
    #   claude --max-turns 3 -p x --model NOT-A-REAL-MODEL --output-format json
    #     -> rc 1, `[claude-code:unrecognized_model]`, and the JSON result carries
    #        duration_api_ms 0 / total_cost_usd 0 / api_error_status 404 -- i.e. --max-turns was
    #        ACCEPTED and parsing reached the model without contacting the API
    ...
    # And the premise re-checked the same day: `claude --help | grep -c max-turns` -> 0, so the
    # control is still genuinely undocumented rather than having quietly been added to the surface.
    ("claude", (), "--max-turns"): {
        "cli": "claude",
        "version": "2.1.258",
        "date": "2026-09-03",
        ...
        "observed": "rc 1 from unrecognized_model, NOT from an unknown option; the control "
                    ...
                    "(re-checked: 0 occurrences in help on 2.1.258)",
    },
}
