...
- [INSTALL.md](INSTALL.md) — setup, including on a new device; the mandatory dual-CLI and Python
  prerequisites; supported-inverse provider recovery and manual recovery-required states;
  recoverable SemVer/version refresh; blocking duplicate-hook migration; isolated loader and rollback
  evidence.
...
- [INTERACTIVE-TEST.md](INTERACTIVE-TEST.md) — drive the firm with a real Claude or Codex session and
  watch the lifecycle engage, including draft/final handoff behavior; it also states what the smoke
  does not prove and which
